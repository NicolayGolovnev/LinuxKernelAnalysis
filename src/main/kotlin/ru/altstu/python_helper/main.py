...
def set_dir(name:str) -> None:
    current_folder = f"{config.result_path}/{analysisData.name}"
...
if __name__ == '__main__':
    ...
    for file_path in os.listdir(config.input_path):
        with open(f"{config.input_path}/{file_path}") as json_config:
            json_data = json.load(json_config)
        analysisData = DirectoryAnalysisData(**json_data)
        set_dir(analysisData.name)
        ...
        print("Covered", get_metric(Z, len(vectors), config.count_clusters_in_sample, threshold)[0], "%")
        print("Fragmentation", get_metric(Z, len(vectors), config.count_clusters_in_sample, threshold)[1], "%")

        plt.figure(figsize=(10, 5))
        plt.title('Hierarchical Clustering Dendrogram')
        plt.ylabel('Distance')
        dend = dendrogram(Z, color_threshold=threshold, no_labels=True)
        plt.axhline(y=threshold, color='r', linestyle='--')
        plt.show()

        X = np.arange(0, 0.8, 0.001)
        Y1 = np.array([get_metric(Z, len(vectors), config.count_clusters_in_sample, x)[0] for x in X])
        Y2 = np.array([get_metric(Z, len(vectors), config.count_clusters_in_sample, x)[1] for x in X])
